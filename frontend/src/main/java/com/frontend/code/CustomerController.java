...
@PostMapping("/login")
public ResponseEntity<Customer> login(@RequestBody Customer user, HttpSession session) {
	...
	if (user == null) {
		...
		return new ResponseEntity<Customer>(user, HttpStatus.UNAUTHORIZED);// 401
	} else {
		userService.update(user);
		session.setAttribute("loggedInUserId", user.getEmail());
		System.out.println("-----Logg in Id :" + user.getEmail());
	}
...
public ResponseEntity<Customer> validate(@RequestBody Customer customer){
	...
	Customer user=userService.findByEmail(email,password);
	if(user!=null)
	{
